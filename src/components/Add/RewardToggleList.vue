<template>
  <div class="reward-toggle-list">
    ...
    ></reward-toggle>
    <reward-custom
      :selected="isCustomRewardValue"
      :model-value="customReward"
      @update:modelValue="
        (value) => {
          updateReward(value);
        }
...
import RewardToggle from "@/components/Add/RewardToggle.vue";
import RewardCustom from "@/components/Add/RewardCustom.vue";
...
const isCustomRewardValue = computed(() => {
  const selectedRewardIndex = rewards.value.findIndex(
